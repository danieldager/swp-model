#!/usr/bin/env python3
"""
Generate ablation search plots (Figure 4).
...
def generate_ablation_search(
    ...
    """Generate ablation search plots."""
    # Setup
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate ablation search plots (Figure 4)"
    )
    ...
    parser.add_argument(
        "--include_stress",
        action="store_true",
        help="Include stress in ablation search",
    )
    parser.add_argument(
        "--regenerate",
        action="store_true",
    ...
    weights_path = args.weights_path or paths["weights"][str(args.batch_size)]
    data_dir = args.data_dir or paths["data"]
    figures_dir = args.figures_dir or paths["figures"]

    # Generate plots
    generate_ablation_search(
