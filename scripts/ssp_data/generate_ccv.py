...
consonant_data = {
    ...
    "Manner of Articulation": [
        ...
        "Nasal",
        "Liquid",
        "Liquid",  # 'l' and 'ɹ' under Liquids
        "Glide",
        "Glide",
        "Glide",  # Separate `(w)` for Bilabial and Velar
    ],
    "Place of Articulation": [
        ...
        "Palatal",
        "Bilabial",
        "Velar",  # `(w)` in both Bilabial and Velar
    ],
}
...
consonant_manner_map = consonant_df.groupby("Consonants").first()[
    "Manner of Articulation"
]
...
for C1 in consonant_df["Consonants"]:
    for C2 in consonant_df["Consonants"]:
        if C1 != C2:  # avoids repetitions
            ...
            if (
                consonant_df[consonant_df["Manner of Articulation"] == M1].index[0]
                > consonant_df[consonant_df["Manner of Articulation"] == M2].index[0]
            ):
...
ccv_non_ssp_df["CCV"] = (
    ccv_non_ssp_df["C1"] + ccv_non_ssp_df["C2"] + ccv_non_ssp_df["V"]
)
...
# All together now
ccv_combined_df = pd.concat(
    [
        ccv_ssp_df[["CCV", "C1", "C1 Manner", "C2", "C2 Manner", "V", "SSP Status"]],
        ccv_non_ssp_df[
            ["CCV", "C1", "C1 Manner", "C2", "C2 Manner", "V", "SSP Status"]
        ],
    ],
    ignore_index=True,
)
...
ccv_combined_df["Sonority Difference"] = ccv_combined_df.apply(
    lambda row: calc_sonority_diff(row["C1 Manner"], row["C2 Manner"]), axis=1
)
