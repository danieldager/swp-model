#!/usr/bin/env python3
"""Sanity check for the Step 2 audio model core.

Tests (no real data required — uses synthetic waveforms):
  1. EnCodec loads from HuggingFace
  2. reconstruct() runs on a synthetic 1s waveform and returns finite audio
  3. extract_activations() works for all 3 stable layer names
...
Run from swp-model/ repo root:
    ...
    python scripts/audio/sanity_check.py --bandwidth 1.5
"""
...
def main(bandwidth: float) -> None:
    print(f"── Sanity check: EnCodec (bandwidth={bandwidth}) ──────────────────────")

    # ── Import ────────────────────────────────────────────────────────────────
    print("1. Importing swp.audio.models ...")
    # Importing encodec.py registers the model in the registry
    import swp.audio.models.encodec  # noqa: F401
    from swp.audio.models import get_model
    print("   OK")

    # ── Load model ────────────────────────────────────────────────────────────
    print("2. Loading EnCodec model ...")
    model = get_model("encodec", bandwidth=bandwidth)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=6.0,
        help="EnCodec bandwidth in kbps (default: 6.0)",
    )
