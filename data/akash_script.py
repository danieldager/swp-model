...
def generate(
    ...
    w = sum([fnt.getbbox(t)[2] - fnt.getbbox(t)[0] for t in text])
    h = sum([fnt.getbbox(t)[3] - fnt.getbbox(t)[1] for t in text]) / len(text)
    ...
    for t in text:
        ...
        letter_w = (
            fnt.getbbox(t)[2] - fnt.getbbox(t)[0]
        )  # Use the letter width to move the following
        x += letter_w + spacing
...
def create_wordset(path_out="./graphemes_data/", ngrams=1, words=["a"]):
    ...
    latents_size = [
        ...
        len(fonts),
        len(uppers),
    ]  # + [len(set(x)) for x in classes.values()]
    ...
    np.savez(
        ...
        latents_values_str=latents_values_str,
    )
